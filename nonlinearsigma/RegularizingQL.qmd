...
Where $Q_{L} \Delta$ is a local charge defined over a triangle, in a technique developed by [Berg and Lüscher, 1981](https://doi.org/10.1016/0550-3213(81)90568-X) to regularize the charge (i.e. ensure that it is always an integer). This technique is detailed below, but involves enforcing the fields to obey certain configurations to ensure that the topological charge is a conserved, integer quantity.
...
<div class="alert alert-warning"> Add a figure here that shows the rate at which our code produces exceptional triangles. Does it vary by lattice size? Analyze how much that will affect our statistics if we have to throw away any data where there are non integer Q
</div>
